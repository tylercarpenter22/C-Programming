...
//---------------------------------------------------------------NOTES TO HELP UNDERSTAND HOW TO CHECK BALANCE OF PARENTHESIS AND BRACKETS ALONG WITH INFIX AND POSTIFX PROCESS---------------------------
//--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
/* 
        ...
        If the priority of the current operator is same or lower than the priority of the operator at the top of the stack, we keep POPing operators from the stack until the priority 
            of the current operator is higher than the operator at the top of the stack. POPed operators are appended to the Postfix in the order they are POPed. Finally, the current 
...
// wrote this function to make it easy to check if our current char is an operator 
int isOperator(char ch)
{
...
{   
    ...
    for (int i = 0; i < strlen(infix); i++)
    {
        ...
        else if (isOperator(infix[i]))
        {
            ...
            {
              while(!empty(&operators) && priority(inflix[i]) <= priority(peek(&operators)) && !isParenthesis(peek(&operators))){
...
// -------------------------------------------------------------------Hmmmmmm Long code and logic isn't it? Ok you guys need to do this without my help! (dw I left comments :) )
