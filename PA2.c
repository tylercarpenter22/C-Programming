...
int main() {
  ...
  for (int i = 0; i < numTestCases; i++) {
    ...
    // Create array of pointers to customers
    customer **customerArray = createCustomerArray(numCustomers);

    // Create array of queues
    struct queue *qPtr = (struct queue *)malloc(sizeof(struct queue) * numLines);
    ...
    // Free memory for customer array and individual customers
...
int enqueue(struct queue *qPtr, customer cust) {
  ...
  if (qPtr->back == NULL) {
    ...
    // Otherwise, add the new node to the end of the queue and change the back
    // of the queue.
...
// This function creates an array of pointers to customers
customer **createCustomerArray(int numCustomers) {
  // Allocate space for array of pointers to customers
...
customer getNextCustomer(struct queue *qPtr) {
  ...
  for (int i = 0; i < numLines; i++) {
    if (!empty(&qPtr[i])) {
      currentCustomer = peak(&qPtr[i]);
      if (minSmoothies == EMPTY ||
          currentCustomer.numSmoothies < minSmoothies) {
...
// This function calculates the checkout time for a customer and displays
// output.
int calculateCheckout(customer cust, int currentTime) {
  ...
  printf("At time %d, %s left the counter from line %d.\n", checkoutTime,
         cust.name, cust.lineNum);
  return checkoutTime;
}

// This function clears the queue and frees memory.
void clearQueue(struct queue *qPtr) {
  while (!empty(qPtr)) {
    dequeue(qPtr);
  }
...
void displayCustomers(customer *customers, int numCustomers){
  for(int i = 0; i < numCustomers; i++){
    printf("\n%d %d %s %d\n", customers[i].time, customers[i].lineNum,
customers[i].name, customers[i].numSmoothies);
  }
}
*/
